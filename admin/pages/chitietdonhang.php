...
<?php
  ...
  if(isset($_POST['suachitietsp']))
  {
   ...
   {
      $soluongcu=soluongsanphamdathang($idDH,$idSP);
      capnhatsoluongsanpham($idSP,$soluongcu,$soluongmoi);

     if(capnhatsoluongchitiet($idSP,$soluongmoi,$idDH)==TRUE)
     {
       echo " Loi";
     }
   }
...
$sanphamdathangtheoIdDH=sanphamdathangtheoIdDH($idDH);
$dathangtheoid=dathangtheoid($idDH);
...
<div class="row">
  ...
  <div class="table-responsive">
    ...
    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
      <thead>
        <tr>
          ...
          <th style="text-align: center" <?php if($dh['xacnhan']==1)echo "hidden=''" ?>>Xoá sản phẩm</th>
        </tr>
      </thead>
      <tbody>
        <?php 
        ...
        ?>
          <tr>
            ...
            <td style="text-align: center">
                <input style="max-width:50px"  type="number" name="sanphamsua" disabled="" value="<?=$sanpham['idSP']?>">
            </td>
            <td><?=$sanpham['tensp']?></td>
            <td style="text-align: center">
                <input style="max-width: 70px" type="number" min="1" max="<?=$sanpham['slspDB']+$sanpham['soluong']?>" name="soluong[<?=$sanpham['idSP']?>]" value="<?=$sanpham['soluong']?>" >
            ...
            <td style="text-align: center"><a <?php if($dh['xacnhan']==1)echo "hidden=''" ?> href="pages/xoaspdonhang.php?idDH=<?=$idDH?>&idSP=<?=$sanpham['idSP']?>">Xoá</a></td>
          </tr>
    ...
    <div style="text-align: center"><button  <?php if($dh['xacnhan']==1)echo "disabled=''" ?>  class="btn btn-success" type="submit" name="suachitietsp">Sửa</button></div>
    </form>
  </div>
  </div>
</div>
