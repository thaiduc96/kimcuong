...
<?php
	if(isset($_GET['idSP']))
	{
		$flag=0;
		$idSP=$_GET['idSP'];
		$sanphamvachitiettheoidSP=sanphamvachitiettheoidSP($idSP);
    ...
    <div class="col-md-7 col-sm-12 col-xs-12">
        <!-- Advanced Tables -->
	           ...
	           <div class="form-group">
	                ...
	                <?php $danhsachtheloai=danhsachtheloai();  
	                while($theloai=mysql_fetch_array($danhsachtheloai))
	                {
	                	if($theloai['idTL']==$sanpham['idTL'])
	                	{ ?>
	                		<option selected="" value="<?=$theloai['idTL']?>"><?=$theloai['tenTL']?></option>	
						<?php
	                	}else{ ?>
	            ...
	            <div class="form-group">
	                ...
	                <?php
	                	$loaisanphamtheotheloai=loaisanphamtheotheloai($sanpham['idTL']);
	                	while($loaisanpham=mysql_fetch_array($loaisanphamtheotheloai))
	                	{
	                		if($loaisanpham['idloaiSP']==$sanpham['idloaiSP'])
		                	{ ?>
		                		<option selected="" value="<?=$loaisanpham['idloaiSP']?>"><?=$loaisanpham['tenloaiSP']?></option>	
							<?php
		                	}else{ ?>
	            ...
	            <div class="form-group">
	                ...
	                <input type="text" class="form-control" value="<?=$sanpham['tenSP']?>" name="tenSP" placeholder="Tên sản phẩm" />
	            </div>
	        	<div class="form-group">
	                <label>Giá</label>
	                <input class="form-control" value="<?=$sanpham['gia']?>"  type="number" min="100000" max="999999999999" name="gia" placeholder="Giá" />
	            </div>

	            <div class="form-group">
	                <label>Khuyến mãi</label>
	                <input class="form-control" value="<?=$sanpham['khuyenmai']?>"  type="number" min="0" max="100" name="khuyenmai" placeholder="Phần trăm khuyến mãi" />
	            </div>

	            <div class="form-group">
	                <label>Số lượng sản phẩm</label>
	                <input class="form-control"  type="number" value="<?=$sanpham['soluong']?>" min="0" max="100" name="soluong" placeholder="Số lượng sản phẩm ban đầu" />
	            </div>

	            <div class="form-group">
	                <label>Tóm tắt</label>
	                <textarea class="form-control" rows="3"  name="txtTomtat" placeholder="Tóm tắt ngắn gọn về sản phẩm"><?=$sanpham['tomtat']?></textarea>
	            </div>
    ...
    <div class="col-md-5 col-sm-12 col-xs-12">
    	<div class="panel-body panel">
    		 ...
    		 <div class="form-group">
	                ...
	                <input class="form-control" value="<?=$sanpham['trongluong']?>" name="trongluong"  placeholder="G" />
	            </div>
	        	<div class="form-group">
	                <label>Chủng loại</label>
	                <input class="form-control" value="<?=$sanpham['chungloai']?>"  name="chungloai" placeholder="Nhẫn, dây chuyền, lắc tay ,...." />
	            </div>

	            <div class="form-group">
	                <label>Tuổi</label>
	                <input class="form-control" value="<?=$sanpham['tuoi']?>" name="tuoi" placeholder="Tuổi vàng" />
	            </div>

	            <div class="form-group">
	                <label>Màu chất liệu</label>
	                <input class="form-control" value="<?=$sanpham['mauchatlieu']?>"  maxlength="10" name="mauchatlieu" placeholder="Trắng, vàng,..." />
	            </div>

	            <div class="form-group">
	                <label>Mô tả</label>
	                <textarea class="form-control" rows="3" name="mota" placeholder="Mô tả sản phẩm hiển thị ở thông tin chi tiết sản phẩm..." ><?=$sanpham['soluong']?></textarea>
	            </div>
...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
            ...
            <textarea class="form-control " id="noidung" rows="3" name="noidung"><?=$sanpham['soluong']?><?=$sanpham['noidung']?></textarea>    
        </div> 	
