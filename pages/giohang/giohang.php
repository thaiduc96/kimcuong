...
<div class="span9">
	...
	<?php
		...
		<form method="post" action="#">
			...
			<tr><th> ĐĂNG NHẬP ĐỂ MUA HÀNG  </th></tr>
			 <tr> 
			 <td>
				<form class="form-horizontal">
					...
					<div class="control-group">
					  <div class="controls">
						<button type="submit" name="dangnhap" class="btn">Đăng nhập</button> hoặc <a href="index.php?p=dangki" class="btn">Đăng kí ngay!</a>
					  </div>
					</div>
					<div class="control-group">
						<div class="controls">
						  <a href="index.php?p=quenmatkhau" style="text-decoration:underline">Quên mật khẩu ?</a>
						</div>
					</div>
				</form>
			  </td>
		...
		//print_r($_SESSION['giohang']);
		{
              ...
              ?>
				<tr>
				  ...
				  <td>
					...
					<div class="input-append">
						<input name="<?=$row['idSP']?>" min="1" max="<?=$soluongsptrongdb?>" idSanPham="<?=$row['idSP']?>" class="form-control input-number quantity" style="max-width:34px" value="<?=$soluong?>"  size="16" type="number">
						<button type="button" class="btn btn-default btn-number" data-type="minus" data-field="<?=$row['idSP']?>"> <i class="icon-minus"></i></button>
						<button type="button" class="btn btn-default btn-number" <?php if($soluong==$soluongsptrongdb)echo " disabled=''" ?> data-type="plus" data-field="<?=$row['idSP']?>"><i class="icon-plus"></i></button>
						<a href="pages/giohang/xoasanphamgiohang.php?idSP=<?=$row['idSP']?>"><button class="btn btn-danger" type="button"><i  class="icon-remove icon-white"></i></button></a>
					</div>
				  </td>	
                  <td><?=$row['gia']?></td>
                  <td><?php if($row['khuyenmai']!=0) {echo khuyenmai($row['gia'],$row['khuyenmai']);}  else echo 0; ?></td>
                ...
                <tr>	
                 ...
                 <td class="label label-important" style="display:block"> <strong>  <?=$tong-$tongkm?> </strong></td>
                </tr>
            ...
            <a href="index.php?p=muahang" class="btn btn-large pull-right">Mua hàng... <i class="icon-arrow-right"></i></a>
             <a href="index.php" class="btn btn-large"><i class="icon-arrow-left"></i> Quay về trang chủ để tiếp tục mua hàng </a>
		<?php
			}
			else{
				?> 
